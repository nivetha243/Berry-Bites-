...
// Importing product images (as before)
...
// Mock product data object (as before)
const productData = {
  'Acrylic Paint': {
    img: Product1,
    price: '$19.99',
...
const isAuthenticated = () => {
    ...
    return !!authToken; // Check if the auth token exists and return true or false
  };
  ...
  const ProductDetail = () => {
    ...
    const buyNow = () => {
      if (isAuthenticated()) {
        ...
        // Redirect to sign-in page if not authenticated
        alert('You need to sign in to proceed with the purchase!');
        navigate('/signin');
      }
    };
